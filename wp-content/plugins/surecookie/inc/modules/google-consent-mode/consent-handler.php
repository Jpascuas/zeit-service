...
class Consent_Handler {
	...
	private function generate_consent_script(): string {
		// Parse consent cookie with security validation (Critical Fix #6).
		$consent_cookie = $this->parse_consent_cookie();
		...
		if ( $gpc_enabled ) {
			// Respect GPC signal - deny all tracking.
			$preferences = [
				'essential'  => true,
				'functional' => false,
				'analytics'  => false,
		...
		} elseif ( $consent_cookie && isset( $consent_cookie['preferences'] ) ) {
			// User has existing consent.
			$preferences = $consent_cookie['preferences'];
		} else {
			// First-time visitor - deny all except essential.
			$preferences = [
				'essential'  => true,
				'functional' => false,
				'analytics'  => false,
		...
		$output .= $this->generate_region_consent_defaults( $consent_cookie, $gpc_enabled, $wait_time, $json_flags );
	 ...
	 *
	 * @param array<string, mixed>|null $consent_cookie Parsed consent cookie or null.
	 ...
	 * @since 0.0.1-beta.1
	 */
	private function generate_region_consent_defaults( $consent_cookie, $gpc_enabled, $wait_time, $json_flags ): string {
		...
		if ( $gpc_enabled || ( $consent_cookie && isset( $consent_cookie['preferences'] ) ) ) {
			return '';
		}
	...
	/**
	 * Parse consent cookie with security validation.
	 *
	 * Critical Fix #6: Secure cookie parsing.
	 *
	 * @return array<string, mixed>|null Parsed consent data or null if invalid.
	 * @since 0.0.0-alpha.1
	 */
	private function parse_consent_cookie(): ?array {
		if ( ! isset( $_COOKIE['surecookie_user_consent'] ) ) {
			return null;
		}

		// Correct order: unescape, decode, then sanitize.
		$raw_value     = wp_unslash( $_COOKIE['surecookie_user_consent'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized immediately via wp_unslash → urldecode → sanitize_text_field.
		$decoded_value = urldecode( $raw_value );
		$cookie_value  = sanitize_text_field( $decoded_value );
		...
		// Validate structure.
		if ( ! is_array( $decoded ) ||
			! isset( $decoded['preferences'] ) ||
			! is_array( $decoded['preferences'] ) ) {
			return null;
		}

		// Validate required preference keys and their types.
		$required_keys = [ 'essential', 'functional', 'analytics', 'marketing' ];
