...
class Consent_Handler {
	...
	public function sync_consent_state(): void {
		...
		$preferences       = $this->parse_consent_cookie();
	...
	/**
	 * Parse the SureCookie consent cookie.
	 *
	 * Reuses the same parsing logic as the GCM Consent_Handler for consistency.
	 *
	 * @return array<string, bool>|null Preferences array or null if no cookie.
	 * @since 0.0.1-beta.1
	 */
	private function parse_consent_cookie(): ?array {
		...
		/**
		 * Unslash → URL-decode → JSON parse.
		 *
		 * Note: sanitize_text_field is intentionally omitted because it can corrupt URL-encoded JSON (e.g., stripping %XX sequences). Instead, we rely on json_decode + structural validation + boolval casting below for safe data handling.
		 *
		 * @since 0.0.1-beta.1
		 */
		$raw_cookie   = wp_unslash( $_COOKIE['surecookie_user_consent'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- JSON is validated structurally below; sanitize_text_field corrupts URL-encoded JSON.
		$cookie_value = urldecode( $raw_cookie );
		...
		// Validate that all default category keys are present.
		$required_keys = [ 'essential', 'functional', 'analytics', 'marketing' ];
		foreach ( $required_keys as $key ) {
			if ( ! isset( $decoded['preferences'][ $key ] ) ) {
				return null;
			}
		}

		// Cast all preference values to boolean to prevent truthy-string bypass.
		return array_map( 'boolval', $decoded['preferences'] );
	}
}
