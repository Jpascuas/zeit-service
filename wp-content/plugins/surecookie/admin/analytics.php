...
class Analytics {
	...
	public function __construct() {
		...
		if ( defined( 'SURECOOKIE_PRO_VERSION' ) ) {
			$deactivation_surveys[] = [
				...
				'popup_title'       => 'Quick Feedback',
				'support_url'       => Helper::get_marketing_link( 'contact/', 'deactivation_survey_support' ),
				'popup_description' => 'If you have a moment, please share why you are deactivating SureCookie Pro:',
				'show_on_screens'   => [ 'plugins', 'plugins-network' ],
	...
	private function detect_state_events(): void {
		...
		$events->track( 'plugin_activated', SURECOOKIE_VERSION, [ 'source' => $source ] );
		...
		if ( get_option( SURECOOKIE_ONBOARDING_COMPLETED_OPTION, false ) ) {
			$events->track( 'onboarding_completed', SURECOOKIE_VERSION );
		}
		...
		if ( ! empty( $nudges['upgrade_banner']['count'] ) ) {
			$events->track(
				...
				(string) $nudges['upgrade_banner']['count']
			);
		}
		...
		if ( ! empty( $nudges['consent_log_report']['count'] ) ) {
			$events->track(
				...
				(string) $nudges['consent_log_report']['count']
			);
		}
		...
		$policy_page_id = (int) Settings::get( 'cookie_policy_page_id' );
		if ( $policy_page_id > 0 ) {
			$events->track( 'cookie_policy_page_configured', (string) $policy_page_id );
		}
